...
export default function SignupPage() {
  ...
  const inputBase = 'w-full px-4 py-2.5 rounded-xl text-sm text-[#fef3c7] placeholder-[#292524] outline-none transition-all duration-200'
  const iS: React.CSSProperties = { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }
  const iF: React.CSSProperties = { border: '1px solid rgba(245,158,11,0.5)', boxShadow: '0 0 0 3px rgba(245,158,11,0.08)' }
  const iB: React.CSSProperties = { border: '1px solid rgba(255,255,255,0.08)', boxShadow: 'none' }
    ...
    <div className="min-h-screen bg-[#0c0c0e] flex flex-col">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div
          ...
          style={{ background: 'radial-gradient(ellipse at center,rgba(245,158,11,0.10) 0%,transparent 70%)' }}
        />
        <div
      ...
      {/* ── Navbar ── */}
      <nav className="relative z-10 px-6 py-4" style={{ borderBottom: '1px solid rgba(245,158,11,0.1)' }}>
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-[#57534e] text-sm">Start your chess journey today</p>
          </div>
          ...
          >
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-[#fef3c7] transition-all duration-200 hover:bg-white/[0.06] active:scale-[0.98] mb-6 disabled:opacity-60"
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <span className="text-xs text-[#44403c]">or register with email</span>
              <div className="flex-1 h-px" style={{ background: 'rgba(255,255,255,0.06)' }} />
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* Username */}
              <div>
                <label className="block text-sm font-medium text-[#a8a29e] mb-1.5">Username</label>
                <input
                  type="text" name="username" value={form.username} onChange={handleChange}
                  placeholder="GrandKnight99" required minLength={3} maxLength={20}
                  className={inputBase} style={iS}
                  onFocus={e => Object.assign(e.target.style, iF)}
                  onBlur={e => Object.assign(e.target.style, iB)}
                />
                <p className="text-xs text-[#292524] mt-1">3–20 characters · Your public display name</p>
              </div>

              {/* Email */}
              <div>
                <label className="block text-sm font-medium text-[#a8a29e] mb-1.5">Email address</label>
                <input
              ...
              {/* Password */}
              <div>
                <label className="block text-sm font-medium text-[#a8a29e] mb-1.5">Password</label>
                <div className="relative">
                  <input
                    type={showPassword ? 'text' : 'password'} name="password"
                    value={form.password} onChange={handleChange}
                    placeholder="••••••••" required minLength={8}
                    className={`${inputBase} pr-11`} style={iS}
                    onFocus={e => Object.assign(e.target.style, iF)}
                    onBlur={e => Object.assign(e.target.style, iB)}
                  />
                    ...
                    <div className="flex gap-1 mb-1">
                      {[1, 2, 3, 4].map(i => (
                        <div key={i} className="h-1 flex-1 rounded-full transition-all duration-300"
                          style={{ background: i <= pw.score ? pw.color : 'rgba(255,255,255,0.07)' }} />
                      ))}
                    </div>
                    <p className="text-xs font-semibold" style={{ color: pw.color }}>{pw.label} password</p>
                  </div>
                )}
              </div>
              ...
              {/* Confirm password */}
              <div>
                <label className="block text-sm font-medium text-[#a8a29e] mb-1.5">Confirm password</label>
                <div className="relative">
                  <input
              ...
              <div className="flex items-start gap-3 pt-1">
                <button type="button" onClick={() => setAgreed(!agreed)}
                  ...
                  style={{
                    background: agreed ? 'linear-gradient(135deg,#f59e0b,#d97706)' : 'rgba(255,255,255,0.03)',
                    border: agreed ? 'none' : '1px solid rgba(255,255,255,0.12)',
                  }}>
                  {agreed && (
                    <svg className="w-2.5 h-2.5 text-[#0c0c0e]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                ...
                <span className="text-xs text-[#44403c] leading-relaxed">
                  I agree to the{' '}
                  <Link href="/terms" className="text-[#f59e0b] hover:text-[#fbbf24] underline">Terms of Service</Link>
                  {' '}and{' '}
                  <Link href="/privacy" className="text-[#f59e0b] hover:text-[#fbbf24] underline">Privacy Policy</Link>
                </span>
              </div>
              ...
              <button
                type="submit"
                disabled={loading || !agreed || mismatch || form.password.length < 8}
                ...
                style={{
                  ...
                  boxShadow: loading || !agreed || mismatch ? 'none' : '0 0 24px rgba(245,158,11,0.3)',
                }}
              >
                {loading ? (
